...
def get_app():
    ...
    def update_ticket():
        ...
        try:
            ...
            web_error_logger.error("Не удалось обработать сообщение. %s", error_message)
            return 'Не удалось обработать сообщение.', 400
        # если JSON найден
        if json_start != -1:
            json_str = message[json_start:]
        else:
            ...
            try:
                ...
                try:
                    ...
                    for header_footer in header_footer_elements:
                        ...
                        if header_footer.find('name').text == assignee_name:
