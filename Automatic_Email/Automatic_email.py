...
def send_notification_v2(version):
    """
    Функция по отправке рассылки клиентам.
    Информация берётся из сервиса Крег, в котором есть перечень клиентов,
    кому отправлять рассылку. Перебираются клиенты и перебираются контакты,
    находится "Основной" и "Копия" кому отправляется письмо с рассылкой.
    На себя принимает параметр version, который подставляется в HTML шаблон,
    ...
    try:
        ...
        response = requests.get('https://creg.boardmaps.ru/api/version2_clients', auth=auth)
        ...
        for client in clients_data:
            if client['contact_status']:
                ...
                with open('HTML/index_2x.html', 'r') as file:
...
def send_notification_v3(version):
    """
    Функция по отправке рассылки клиентам.
    ...
    try:
        ...
        auth = (CREG_USERNAME, CREG_PASSWORD)
        # Получаем данные для определения информации кому отправлять рассылку
        response = requests.get('https://creg.boardmaps.ru/api/version3_clients', auth=auth)
