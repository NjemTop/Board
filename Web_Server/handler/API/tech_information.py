...
def patch_tech_information(client_id):
    """
    Функция обновления данных технической информации о клиенте.
    В URL запросе указываем id клиента, которому нужно обновить информацию после /
    Обновить можно как всю сразу информацию, так и только номер версии.
    Если обновляем номер версии, то можно не указывать дату обновления, 
    в таком случае дата будет проставлено автоматически из сегоднешнего дня.
    """
    # Проверяем существование клиента с указанным client_id
    try:
        with conn:
