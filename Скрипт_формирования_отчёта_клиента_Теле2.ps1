### ПРОВЕРКА РЕАКЦИИ PowerShell, В КОТОРОЙ ВЫПОЛНЯЕТСЯ СКРИПТ.
### ЕСЛИ СКРИПТ ВЫПОЛНЯЕТСЯ powershell.exe, В НЕ pwsh.exe, СКРИПТ ПЕРЕЗАПУСКАЕТСЯ в pwsh.exe
...
try {
    ### СОЗДАДИМ ОБЪЕКТ WORD ТАБЛИЦЫ
    $WORD = New-Object -ComObject Word.Application
    # ! $WORD.Visible = $true

    ### СОЗДАДИМ ЛИСТ ДЛЯ ЗАПОЛНЕНИЕ ДАННЫХ
    $DOCUMENT = $WORD.Documents.Add()
    ### ВЫБЕРИМ НАШИ ЛИСТ, В КОТОРУЮ БУДЕМ ЗАПИСЫВАТЬ ИНФУ
    $SELECTION = $WORD.Selection
    ### УСТАНОВИМ СТИЛЬ ШРИФТА
    $SELECTION.Font.Name = "Times New Roman"
    ### УСТАНОВИМ ОТСТУПЫ ДЛЯ ДОКУМЕНТА
    ...
    ### ЗАДАДИМ ШАПКЕ ЖИРНЫЙ ТЕКСТ
    $SELECTION.Font.Bold = $True
    ### РАЗМЕР ШРИФТА
    $Selection.Font.Size = 14
    ### ЗАДАДИМ НАЗВАНИЕ ШАПКИ
    $SELECTION.TypeText("ОТЧЕТ ОБ ОКАЗАННЫХ УСЛУГАХ")
    ### РАЗБИВАЕМ ТЕКСТ НА СТРОКИ (ЭНТЕР МЕЖДУ СЛОВАМИ)
    ...
    $SELECTION.TypeText("от $TODAY_DAY $TODAY_Month $TODAY_YEAR г. № 9")
    $SELECTION.TypeParagraph()
    $SELECTION.TypeParagraph()
    ### УБЕРЁМ ЖИРНОСТЬ ТЕКСТА
    $SELECTION.Font.Bold = $false
    ### РАЗМЕР ШРИФТА
    $Selection.Font.Size = 12
    $SELECTION.TypeText("по Договору № 03/04/20 от «10» апреля 2020г.")
    $SELECTION.TypeParagraph()
    $SELECTION.TypeText("за период с «___» ___ ___ г. по «$TODAY_DAY» $TODAY_Month $TODAY_YEAR г.")
    $SELECTION.TypeParagraph()
    $SELECTION.TypeParagraph()
    ### СОЗДАЁМ ТАБЛИЦУ И ЗАДАДИМ РАЗМЕРЫ ТАБЛИЦЫ
    $Number_Of_Rows = 1
    $Number_Of_Columns = 8
    ...
    ### СДЕЛАЕМ ТАБЛИЦУ ВИДИМОЙ
    $table.Borders.Enable = 1
    ### ЗАДАДИМ НАЗВАНИЕ ШАПКИ ТАБЛИЦЫ
...
try {
    ...
    for ($PAGE = 1; $PAGE -le $GET_JSON_REPORT_TELE2.page_count; $PAGE++) {
        ...
        try {
            ...
            $PS_ERRORS = New-Object PSObject
            $PS_ERRORS | Add-Member -Type NoteProperty "Действие" -Value "Добавления тикета в таблицу"
            $PS_ERRORS | Add-Member -Type NoteProperty "Тикет ID" -Value "$($GET_JSON_RESPONSE_TICKET.id)"
            $PS_ERRORS = $TABLE_ERRORS.Add($PS_ERRORS)
            Write-Host -ForegroundColor Red -Object "Ошибка добавления тикета: $($GET_JSON_RESPONSE_TICKET.id)"
        }
    }
}
catch {
...
try {
    ...
    $TEMP_WORD_FILE = "$env:LOCALAPPDATA\Отчёт_клиента_Теле2.docx"
...
try {
    ...
    if ($BODY) {
        ...
        try {
            $SHARE_SAVE = "D:\Script\BoardMaps\bot-tg-main\bot-tg-main\BM_bot\Теле2"
