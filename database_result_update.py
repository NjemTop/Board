...
def upload_db_result(version_SB, result_SB):
    ...
    for i in item:
        ...
        if i != '':
            ...
            map[id]["Номер_релиза"] = version_SB
    ...
    conn.commit()
    # Наполняем БД
    df = pd.DataFrame(result, columns = ["Дата_рассылки", "Номер_релиза", "Наименование_клиента", "Основной_контакт", "Копия"])
